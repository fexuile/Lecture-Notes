...
def cornersHeuristic(state, problem):
    ...
    if(len(Need_to_Visit)==0):return 0
    h_sum = 100000000
    # for n in corners:
    #     if not n in Visited_Corners:
    #         h_sum = max(h_sum,abs(node[0]-n[0])+abs(node[1]-n[1]))
    for step in itertools.permutations(Need_to_Visit):
        s = abs(node[0]-step[0][0])+abs(node[1]-step[0][1])
        for i in range(1,len(step)):
            s += abs(step[i-1][0]-step[i][0])+abs(step[i-1][1]-step[i][1])
        h_sum = min(h_sum, s)
    return h_sum 
