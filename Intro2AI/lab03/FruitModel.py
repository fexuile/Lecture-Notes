...
class NaiveBayesModel:
    ...
    def count(self):
        for tokens, label in self.dataset:
            for token in tokens:
                self.pos_neg_num[label] += 1
                self.token_num[label][token] = 1 + self.token_num[label].get(token,0)
                self.V += 1
    ...
    def __call__(self, text):
        ...
        num=len(text)
        for label in [0, 1]:
            log_prob[label] = math.log(self.pos_neg_num[label] / sum(self.pos_neg_num))
            for token in text:
                log_prob[label] += math.log((self.token_num[label].get(token,0) + self.alpha) / (self.pos_neg_num[label] + num * self.alpha))
        if log_prob[1] > log_prob[0]: return 1
        else: return 0
...
def buildGraph(dim, num_classes): #dim: 输入一维向量长度， num_classes:分类数
    ...
    nodes = [Linear(dim,dim//2),
             BatchNorm(dim//2),
             relu(),
             Linear(dim//2, num_classes),
             LogSoftmax(),
             NLLLoss(0)
             ]
    graph = Graph(nodes)
...
class Embedding():
    ...
    def __call__(self, text):
        vectors=[self.emb.get(word, np.zeros_like(next(iter(self.emb.values())))) for word in text]
        vector = np.mean(vectors,axis=0)
        return vector.reshape(1,100)
...
class QAModel():
    ...
    def idf(self, word):
        ...
        for document in self.document_list:
            if word in document: d += 1
        return math.log(len(self.document_list) / (d + 1))
    ...
    def __call__(self, query):
        ...
        for document in self.document_list:
            sum = 0
            for word in query:
                sum += self.tfidf(word, document)
            if sum > mx:
                mx = sum
                ans_docu = document
        sentences = ans_docu['sentences']
        ans_sen = None
        mx = -1
