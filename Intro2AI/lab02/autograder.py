...
parser.add_argument(
    "--q", choices=["q1", "q2", "q3", "q4", "all"], default="all")
...
def q3debug():
    ...
    for filename in ["CELoss", "NLLLoss", "Linear", "multilayer", "BatchNorm", "Dropout"]:
        output = programcall(
            f"{PYTHON_PATH} debug_NeuralNetwork.py input/nn.{filename}.in --load")
    ...
    for filename in ["relu", "sigmoid", "tanh", "mix"]:
        output = programcall(
            f"{PYTHON_PATH} debug_ScalarFunction.py input/sf.{filename}.in")
...
def runq4test(cmd, i):
    ...
    output = programcall(cmd)
    score = filterpacmanscore(output)
    print(f"pacman score {score:.0f}")
    return scorer(score, stardartscore[i], zeroscore[i], fullscorelist[i])
...
def q4():
    ...
    results = []
    p = mp.Pool(repeat * len(mapnamelist))
    for i in range(len(mapnamelist)):
        for seed in range(repeat):
            results.append(p.apply_async(runq4test, args=(
                f"{PYTHON_PATH} pacman.py -p ReflexAgent -a model=\"Your\" -l {mapnamelist[i]} " +
                f"--maxstep {maxsteplist[i]} -q --seed {seed} ", i)))
