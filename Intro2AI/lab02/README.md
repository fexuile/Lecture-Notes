...
1. answerLogisticRegression 15 行注释
@return: 输出标签为正的概率。-> @return: wx+b 

2. input/nn.Linear.in与nn.multilayer.in除去Logsoftmax前的relu。

与问题无关的修正 

1. layout.py 146行的random.choice换成np.random.choice 
2. 增加了pdf版本的README

v2.0:
与问题无关的修正:

1. q3过去的debug_*只检查数值梯度=反向传播梯度。现在还会检查绝对数值。
