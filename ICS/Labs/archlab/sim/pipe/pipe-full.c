...
long long gen_d_valA()
{
    return (((if_id_curr->icode) == (I_CALL) || (if_id_curr->icode) == 
      ...
      ((word_t)ex_mem_curr->vale) : ((id_ex_next->srca) == 
        (mem_wb_curr->destm)) ? (mem_wb_curr->valm) : ((id_ex_next->srca)
         == (mem_wb_curr->deste)) ? (mem_wb_curr->vale) : (d_regvala));
}
...
long long gen_d_valB()
{
    return (((id_ex_next->srcb) == (ex_mem_next->deste)) ? 
      ...
      ((word_t)ex_mem_curr->vale) : ((id_ex_next->srcb) == 
        (mem_wb_curr->destm)) ? (mem_wb_curr->valm) : ((id_ex_next->srcb)
         == (mem_wb_curr->deste)) ? (mem_wb_curr->vale) : (d_regvalb));
}

long long gen_aluA()
{
    return (((((((id_ex_curr->icode) == (I_IADDQ)) & ((id_ex_curr->valc)
                 == 1)) & ((ex_mem_curr->icode) == (I_ALU))) & (
            (ex_mem_curr->ifun) == (A_AND))) & (((word_t)ex_mem_curr->vale)
           <= 0)) ? 0 : ((id_ex_curr->icode) == (I_RRMOVQ) || 
...
{
    return ((((id_ex_curr->icode) == (I_RMMOVQ) || (id_ex_curr->icode) == 
          (I_PUSHQ)) & ((ex_mem_curr->destm) == (id_ex_curr->srca))) ? 
...
long long gen_mem_addr()
{
    return (((ex_mem_curr->icode) == (I_RMMOVQ) || (ex_mem_curr->icode) == 
        ...
        (ex_mem_curr->icode) == (I_MRMOVQ)) ? ((word_t)ex_mem_curr->vale)
       : ((ex_mem_curr->icode) == (I_POPQ) || (ex_mem_curr->icode) == 
        (I_RET)) ? (ex_mem_curr->vala) : 0);
}
