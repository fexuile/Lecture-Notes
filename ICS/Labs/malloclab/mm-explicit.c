...
void mm_checkheap(int lineno) {
    ...
    for (ptr = heap_listp; GET_SIZE(HDRP(ptr)) > 0; ptr = NEXT_BLKP(ptr)) {
        ...
        if (!GET_ALLOC(HDRP(ptr))) {
            printf("prev: %x\n", GET_FREE(PREV_FREE(ptr)));
            printf("next: %x\n", GET_FREE(NEXT_FREE(ptr)));
        }
    }
    puts("");
}
...
static void *find_fit(size_t asize) {
    // mm_checkheap(0);
    void *ptr;
    for (ptr = free_listp; !GET_ALLOC(HDRP(ptr)); ptr = GET_FREE(NEXT_FREE(ptr))){
        // printf("nowptr %x\n", ptr);
